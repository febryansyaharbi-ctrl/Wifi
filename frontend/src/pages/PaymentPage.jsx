...
export default function PaymentPage() {
  ...
  useEffect(() => {
    api.get(`/registration/payment-info/${registrationId}`)
        ...
        <div className="mt-8 grid lg:grid-cols-3 gap-6">
          ...
          <div>
            <div className="lg:sticky lg:top-6 bg-white rounded-3xl border border-slate-200 p-6 shadow-sm">
              {reported ? <><CheckCircle2 className="text-emerald-500 mx-auto" size={48}/><h2 className="mt-4 text-center font-bold text-slate-900">Konfirmasi Terkirim</h2><p className="mt-2 text-center text-sm text-slate-500">Status menjadi menunggu verifikasi Super Admin. Bukti pembayaran dikirim melalui WhatsApp.</p></> :
              <><h2 className="font-bold text-slate-900">Konfirmasi Pembayaran</h2><p className="mt-2 text-sm text-slate-500 leading-6">Setelah transfer, klik tombol di bawah. WhatsApp Super Admin akan terbuka dengan pesan yang sudah disiapkan.</p><button onClick={confirm} disabled={busy} className="mt-5 w-full h-12 rounded-xl text-white font-semibold inline-flex items-center justify-center gap-2 disabled:opacity-60" style={{background:"hsl(var(--primary))"}}>{busy ? <Loader2 size={18} className="animate-spin"/> : <MessageCircle size={18}/>} {busy ? "Memproses..." : "Konfirmasi Pembayaran"}</button></>}
            </div>
          </div>
        </div>
      </main>
