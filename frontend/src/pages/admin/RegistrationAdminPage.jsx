          ...
          <div key={a.id} className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
              ...
              <div className="flex gap-2 flex-wrap lg:justify-end">
                {a.status === "PAYMENT_REPORTED" && (
                  <>
                    <button disabled={busy === a.id+"verify"} onClick={() => action(a.id,"verify")} title="Setujui pembayaran yang sudah dilaporkan" className="inline-flex items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-semibold text-white bg-emerald-600 disabled:opacity-40">
                ...
                {a.status === "PENDING_PAYMENT" && (
                  <>
                    <button type="button" disabled title="Tombol aktif setelah pendaftar klik Kirim Bukti Pembayaran" className="inline-flex items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-semibold text-slate-500 bg-slate-100 border border-slate-200 cursor-not-allowed">
                      <CreditCard size={16}/> Menunggu Bukti Pembayaran
                    </button>
                    <button disabled={busy === a.id+"reject"} onClick={() => action(a.id,"reject")} className="inline-flex items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-semibold text-rose-700 bg-rose-50 border border-rose-200 disabled:opacity-50">
                      <XCircle size={16}/> Ditolak
                    </button>
